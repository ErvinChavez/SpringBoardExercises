...
class BinaryTree {
  ...
  static deserialize(string) {
    ...
    if(!string) return null;//return null if there is no string
    ...
    while (queue.length) {//while there are still numbers in queue
     const current = queue.shift();//look at the first number on the queue
    ...
    return new BinaryTree(root);//returns newBinaryTree with root value
